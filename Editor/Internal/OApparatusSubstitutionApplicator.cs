...
{
    public static class OApparatusSubstitutionApplicator
    {
        public static void Apply(GameObject environmentRoot,
                                 OApparatusSubstitutionTable table,
                                 OApparatusObjectSlot[] objectSlots)
        {
            ...
            var instances = environmentRoot
                .GetComponentsInChildren<OApparatusObjectManager>(includeInactive: true);
            ...
            foreach (var instance in instances)
            {
                ...
                if (entry.Prefab == null)
                {
                    if (warnedTypes.Add(objectType))
                        Debug.LogWarning(
                            $"[OpenApparatus] OApparatusSubstitutionTable has an entry for " +
        ...
        // The slot node itself -- transform, OApparatusObjectManager identity -- is
        // left intact; only the visual is swapped.
        static void SwapStandIn(OApparatusObjectManager slot, OApparatusSubstitutionEntry entry)
        {
