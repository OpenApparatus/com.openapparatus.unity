...
{
    /// <summary>
    /// Generates an environment prefab from an <see cref="OApparatusConfig"/>:
    ...
    /// </summary>
    public static class OApparatusPrefabBuilder
    {
        /// <summary>Builds a scene instance from the config (caller owns it).</summary>
        public static GameObject BuildInstance(OApparatusConfig config)
        {
            if (config == null || config.Source == null) return null;

            var options = new OApparatusBuildOptions
            {
        ...
        /// <see cref="OApparatusConfig.GeneratedPrefab"/> at it. Returns the prefab.
        /// </summary>
        public static GameObject Regenerate(OApparatusConfig config)
        {
            if (config == null || config.Source == null) return null;
            ...
            // carrying OApparatusManager with the apparatus geometry as a child.
            var prefabRoot = new GameObject(config.name);
            prefabRoot.AddComponent<OApparatusManager>().Config = config;
        ...
        static void PersistGeneratedAssets(GameObject prefabRoot, OApparatusConfig config)
        {
        ...
        static void Persist(Object obj, OApparatusConfig config, HashSet<Object> seen)
        {
        ...
        static void ApplyRoomConfigs(GameObject root, OApparatusConfig config)
        {
            if (config.Rooms == null) return;
            var envRoot = root.GetComponent<OApparatusRootManager>();
        ...
        static void FillOpenings(GameObject root, OApparatusConfig config)
        {
            var envRoot = root.GetComponent<OApparatusRootManager>();
            if (envRoot == null) return;
        ...
        static void PlaceOpening(Transform roomT, OApparatusWallInfo wall, OApparatusOpeningInfo opening,
        ...
        static void ApplyObjectConfigs(GameObject root, OApparatusConfig config)
        {
                ...
                && config.Source.ObjectSlots != null)
            {
                ...
                if (entries.Count > 0)
                {
                    ...
                    OApparatusSubstitutionApplicator.Apply(root, table, config.Source.ObjectSlots);
            ...
            foreach (var instance in root.GetComponentsInChildren<OApparatusObjectManager>(true))
            {
        ...
        static void RecentreOnOriginRoom(GameObject root, OApparatusConfig config)
        {
        ...
        static Vector3 RoomCentroid(OApparatusRoomManager room)
        {
        ...
        static OApparatusRoomConfig FindRoomConfig(OApparatusConfig config, int roomId)
        {
        ...
        static OApparatusObjectTypeConfig FindObjectConfig(OApparatusConfig config, string objectType)
        {
