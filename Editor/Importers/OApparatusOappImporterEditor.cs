...
{
    [CustomEditor(typeof(OApparatusOappImporter))]
    public sealed class OApparatusOappImporterEditor : ScriptedImporterEditor
    {
        ...
        {
            base.OnEnable();
            _colliderMode = serializedObject.FindProperty(nameof(OApparatusOappImporter.OApparatusColliderMode));
            _substitution = serializedObject.FindProperty(nameof(OApparatusOappImporter.Substitution));
        }

        public override void OnInspectorGUI()
        {
            var importer = (OApparatusOappImporter)target;
            var asset = AssetDatabase.LoadAssetAtPath<OApparatusAsset>(importer.assetPath);
            ...
            using (new EditorGUI.DisabledScope(asset == null))
            {
                if (GUILayout.Button("Create Apparatus", GUILayout.Height(28)))
                    OApparatusWizardOverlay.CreateFor(importer.assetPath, asset);
            }
        }

        static void DrawSummary(OApparatusAsset asset)
        {
            ...
            if (asset.ObjectSlots != null && asset.ObjectSlots.Length > 0)
            {
                ...
                EditorGUILayout.HelpBox(
                    ...
                    + "OApparatusSubstitutionTable entry.",
                    MessageType.None);
