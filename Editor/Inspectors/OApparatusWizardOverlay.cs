...
namespace OpenApparatus.Unity.Editor.Inspectors
{
    ...
    public sealed class OApparatusWizardOverlay : Overlay
    {
        const string OverlayId = "openapparatus-apparatus-setup";

        static OApparatusConfig s_target;
        ...
        /// <summary>
        /// Creates an <see cref="OApparatusConfig"/> for the source apparatus,
        /// generates its prefab, opens that prefab in Prefab Mode, and targets
        /// this overlay at the config.
        /// </summary>
        public static void CreateFor(string sourceAssetPath, OApparatusAsset source)
        {
            if (source == null) return;

            var config = ScriptableObject.CreateInstance<OApparatusConfig>();
            ...
            var prefab = OApparatusPrefabBuilder.Regenerate(config);
            s_target = config;
