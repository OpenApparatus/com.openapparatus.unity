...
{
    public sealed class OApparatusModelBuilderTests
    {
        GameObject _root;
        ...
        static OApparatusParameters Params() => new OApparatusParameters
        {
            TileSize = 3.5f,
            WallThickness = 0.2f,
            WallHeight = 3.0f,
        ...
        public void Build_ProducesOneNodePerRoom()
        {
            ...
            _root = OApparatusModelBuilder.Build("Env", plan, grid, Params(),
                                                  new OApparatusBuildOptions());

            Assert.AreEqual("Env", _root.name);
            Assert.IsNotNull(_root.GetComponent<OApparatusRootManager>());
        ...
        {
            var (plan, grid) = TwoRoomPlan();
            var opts = new OApparatusBuildOptions
            {
                GenerateFloors = true,
                GenerateWalls = false,
                GenerateCeilings = false,
            };
            _root = OApparatusModelBuilder.Build("Env", plan, grid, Params(), opts);
